<template>
    <div class="sapi-select" :class="{ 'readonly': readonly === '' || readonly === true }" :style="{ width }">
        <div @click="handleClick" class="input-wrap" :class="{ 'delete-icon': clearIconFlag, active: visible }">
            <input :readonly="!canInp" :placeholder="choInp || placeholderText" class="sapi-select-input"
                v-model="input" @blur="handleInpBlur" @focus="handleInpFocus" @input="handleInpKey" />
            <i class="icon-caret-right sapi-select-icon" @click.stop="clear" :class="{ 'active': visible }"></i>
        </div>
        <div ref="selectList" :id="selectId" class="sapi-select-box-wrap"
            :style="{ left: sLeft, top: sTop, width: sWidth }" :class="{ active: visible }">
            ...
            <div class="sapi-select-box-list" @mousedown.stop="emptyFn" :class="listClass">
                <sapi-scroll ref="scrollbar" :style="{ maxHeight: maxHeight, height: height }">
                    <sapi-tree :showTip="true" ref="treeRef" :expand-on-click-node="expandOnClickNode"
                        :defaultExpandAll="defaultExpandAll" :actived-id="nodeId" :lazy-load="lazyLoad"
                        :render-content="renderContent" :select-after="getSelectNode"
                        :data="isKeyWrite && input ? searchList : datas" :props="props"
                        @on-click="treeClick"></sapi-tree>
                    <ul v-if="isTree !== true" class="sapi-select-box-list-ul">
                        <template v-if="isKeyWrite && input">
                            <slot :row="item" :$index="index" v-for="(item, index) in searchList">
                                <li class="label" :Key="index" :title="item[props.label]" v-text="item[props.label]"
                                    :class="{ disabled: item[props.disabled] === true, active: item.active }"
                                    @click.stop="nodeClick(item)"></li>
                            </slot>
                        </template>
                        <template v-else>
                    ...
                    <p class="empty-p"
                        v-if="(isKeyWrite && input && searchList.length === 0) || (!(isKeyWrite && input) && datas.length === 0)">
                        无匹配数据</p>
                </sapi-scroll>
            </div>
        </div>
